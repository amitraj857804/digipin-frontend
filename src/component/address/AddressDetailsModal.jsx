...
function AddressDetailsModal({ isOpen, address, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4 top-20 pt-12">
      <div
        className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto scrollbar-hide mb-8"
      ...
      >
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
            <p className="text-sm text-gray-600 font-semibold uppercase mb-3">
              Your Digital Address
            </p>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-2xl font-bold text-blue-600 font-mono break-all">
                  {address.digitalAddress}
                </p>
                <p className="text-xs text-gray-600 mt-2">
                  Use this address to receive deliveries and services
                </p>
              </div>
              <button
                onClick={() =>
                  handleCopy(address.digitalAddress, "Digital Address")
                }
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-900">
                  {new Date(address.createdAt).toLocaleDateString()} at{" "}
                  {new Date(address.createdAt).toLocaleTimeString()}
                </p>
              </div>
            </div>
            {address.updatedAt && (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-900">
                    {new Date(address.updatedAt).toLocaleDateString()} at{" "}
                    {new Date(address.updatedAt).toLocaleTimeString()}
                  </p>
                </div>
              </div>
            )}
